...
-(UIView* )pullDownView{
    if (!_pullDownView) {
        ...
        _pullDownView = [[PullDownView alloc] initWithFrame:CGRectMake((self.frame.size.width-width)*0.5, 0, width, 64)];
        _pullDownView.image = [UIImage imageNamed:@"cm2_discovery_icn_pulldown"];
        
        _pullDownView.label.text = @"首页内容根据你的口味而定";
...
-(void)buttonSetAction{
    if ([self.delegate respondsToSelector:@selector(recommendView:didSelectSetColumButton:)]) {
        [self.delegate recommendView:self didSelectSetColumButton:true];
    }
}
...
-(void)loadColumSetList{
    if ([self.dataArray count] > 0) {
     ...
     [self insertSubview:self.pullDownView atIndex:0];
    }else{
